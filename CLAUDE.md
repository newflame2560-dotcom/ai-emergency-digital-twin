...
An **AI Fire Emergency Digital Twin**, currently **v1.10 · Building Registry — multi-building support**. The frontend is still a single self-contained HTML file with no build step and no framework, backed by real Vercel serverless functions: role-based login, a Claude API proxy, and a Google Maps key proxy. It is a drill/training tool — the login screen and footer both say so — not a live life-safety system, and it must never be presented as one.

As of v1.10 the app is no longer OCC-only: a `BUILDINGS_DATA` registry (see **Building Registry** below) holds per-building floors/rooms/exits/equipment/scenarios, and the user can switch which building is active from the Building Map page. OCC YL (`occ-yl`) is the original, fully-detailed building; อาคาร B โรงซ่อมบำรุง (`depot-b`) is a pilot building with placeholder-but-structured data (not real as-built drawings) that proves the registry works end-to-end. A third box on the Building Map (สถานีไฟฟ้าย่อย / substation) is intentionally **not** in the registry — it stays a read-only info card, out of scope.

The parent workspace rules in `../CLAUDE.md` apply here too (Thai-first, exact law citations, zero-accident standard).

## Files

- `ai-emergency-digital-twin-prototype.html` — **the product** (v1.10). All HTML, CSS, and JS for the app in one file (~3500+ lines). Talks to the backend under `api/`.
...
## Building Registry (v1.10+)

- **`BUILDINGS_DATA`** is the registry: an object keyed by building id (`'occ-yl'`, `'depot-b'`), each holding `floors`, `floorIds`, `floorPlans`, `floorEquip`, `floorRoomMatrix`, `exitVocabulary` (`{ids, twoExitRule}`), `originOptions`, `originRoomResolver`, `corridorCY` (null = no single-corridor model, e.g. depot-b), `scenarioFloorRange`, `trappedFloorOptions`, `drillHistory`/`drillTimeline`/`drillRoles` (OCC-only concept — depot-b ships `[]`/`null`/`null`, pages show "ยังไม่มีประวัติ"), `passThresholdSec`, `storageKey`, `defaultFloorId`/`defaultScenarioFloorId`, and `map`. OCC's constants (`FLOORS`, `FLOOR_PLANS`, `FLOOR_EQUIP`, `FLOOR_ROOM_MATRIX`, `DRILLS`, `OCC_TIMELINE`, `OCC_ROLES`) are defined once and re-wrapped into `BUILDINGS_DATA['occ-yl']` unchanged; depot-b's equivalents are separate `BUILDING_B_*` constants with placeholder-but-structured data (rooms flagged `'Placeholder — pending as-built drawing'`), pending real drawings.
- **`state.activeBuildingId`** (default `'occ-yl'`) selects the active building; `activeBuilding()` reads it. `setActiveBuilding(id)` swaps `STORAGE_KEY` (a top-level `let`, not `const`, so it can be reassigned — required by the marker-based tests, see below), resets and reloads building-scoped state (`scenarios`, `car`, `verify`, `liveDrill`, `lastSim`, `occRun`), and re-renders every page. It's triggered by clicking building A/B on the Building Map (mock SVG or Google Maps marker) and restored on load from its own tiny localStorage key (`aidt_active_building_v1`) — separate from each building's own `storageKey`, so it never goes through `validatePersistedState`.
- **Persistence is per-building**: OCC keeps its original key (`aidt_occ_state_v1`, unchanged — zero migration risk), depot-b uses `aidt_depot_b_state_v1`. Switching buildings is a full state swap, not a merge.
- **Two-exit block UI/logic (`blockW`/`blockE`, "cannot block both") stays OCC-only** (`exitVocabulary.twoExitRule`) — depot-b's 3 ground-level exits (`S1`/`S2`/`S3`, plus `ST1` for the floor-2 stair) don't get route-blocking in this pass; the checkboxes are harmless no-ops if used on depot-b since its exits don't have `id` `W`/`E`.
- **`floorExitNodes()` and `simLoop()`'s corridor movement both dispatch on the active building**: OCC's W/E/L stair-core logic and `CY`-based two-phase corridor movement are untouched (byte-identical, verified against pre-refactor output for scenarios A and G); non-corridor buildings (`corridorCY: null`) use room-flagged `exitNode`/`exitId` and move agents in a straight diagonal line to their exit.
- **`SCENARIO_KEYS`'s `origin` enum is a flat union** of every building's origin values (`svr/offA/offB/meet/lobby` for OCC + `hotwork/lube/battery/mezz` for depot-b) rather than building-conditional — this keeps `validatePersistedState` a pure function of the data (no dependency on `state.activeBuildingId`), which matters both for security (validation shouldn't depend on ambient app state) and because `tests/security-state-validation.test.js` evals `validateScenario` in an isolated VM context that only injects `state`, not `BUILDINGS_DATA`. The floor range (1–5 + `'ดาดฟ้า'`/`'Ground'`) and `trappedFloor` enum needed no change — depot-b's vocabulary (`2`, `'Ground'`, `'ชั้น 2'`) is already a subset of OCC's.
- **⚠️ Marker-based tests**: `tests/security-state-validation.test.js` and `tests/ai-output-validation.test.js` extract source by literal string-slicing between function-name/comment markers (e.g. `between('function saveState(){', 'loadState();')`) and `vm.runInContext` it. Renaming, reordering, or moving `saveState`, `loadState`, `exportState`, `importState`, `esc`, `roomGuidance`, `renderScenarioList`, `logEvent`, `chipHTML`, `buildAARFromSim`, `occAARSections`, `hasExactAIKeys`, `appendAIInsight`, `aiChatContext`, or `AI_CHAT_HISTORY` breaks these tests silently (wrong slice, not a clear error). Run `node --test tests/` after touching anything near those functions.

## Prototype architecture (single `<script>` block, ~line 764 onward)

- **`state.scenarios`**: preset scenarios A/B/C (server-room fire variants) and G/H/I/J (OCC Fire Drill 2568 pack). Scenario Builder edits this list. Scenarios don't carry a `building` field — they live inside whichever building's storage key is currently loaded (see **Building Registry** above).
...
- Bump the version string in the sidebar footer (`ต้นแบบ v1.x`) **and** the sidebar brand `<small>` tag (`... Vector Digital Twin v1.x`) when shipping a new scenario pack or feature set — both currently read v1.10 and must move together.
- Never delete existing OCC floor/room/scenario/drill data when touching the Building Registry — new buildings are additive entries in `BUILDINGS_DATA`, and OCC's constants must stay a pure re-wrap (same values, same objects).
